          ...
          <div className="space-y-5 text-xs">
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                Digital Portals Availability & Access
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {[
                  ...
                  const override = reviewSchool.featureOverrides?.[portal.key];
                  const plan = plans.find(p => p.id === reviewSchool.planId || p.code === (reviewSchool.subscriptionPlan || 'basic').toLowerCase());
                  const isEnabled = override !== undefined ? override : (plan ? plan.features.includes(portal.key as any) : true);

                  return (
                    <div key={portal.key} className="bg-white border border-slate-200 p-2.5 rounded-lg flex items-center justify-between">
                      <span className="font-semibold text-slate-800">{portal.label}</span>
              ...
              <div className="flex items-center gap-2">
                {reviewSchool.status === 'active' && (
                  <>
                    <button
                      onClick={() => {
                        onImpersonateSchool(reviewSchool.id);
                        setReviewSchool(null);
                      }}
                      className="px-3.5 py-2 bg-teal-700 hover:bg-teal-800 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer shadow-2xs"
                    >
